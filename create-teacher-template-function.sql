...
-- This function copies the template subject and all its related data to a new teacher
...
BEGIN
    ...
    -- Copy puzzle games (if any exist)
    ...
    -- Copy question bank entries (if any exist for this teacher)
    ...
    WHERE qb.teacher_id = (SELECT teacher_id FROM subjects WHERE id = template_subject_id)
    AND EXISTS (
        SELECT 1 FROM level_questions lq
        JOIN quiz_levels ql ON lq.level_id = ql.id
        JOIN quiz_games qg ON ql.quiz_game_id = qg.id
        WHERE qg.subject_id = template_subject_id
        AND lq.question_id = qb.id
    );
...
-- Create a simpler function that just takes the teacher_id (uses default template)
